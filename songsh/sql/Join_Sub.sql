...
-- LEFT OUTER JOIN (LEFT JOIN) : 기준 테이블의 모든 레코드와 조합할 테이블의 중 조건에 일치하는 레코드만 반환 
...
-- 서브쿼리 :쿼리 내부에 존재하는 또 다른 쿼리, 쿼리 결과를 조건이나 테이블로 사용할 수 있도록함

-- WHERE 절에서 서브쿼리: 조회 결과를 조건으로 사용하여 조건을 동적으로 지정할수 있도록 함
...
-- WHERE 조건절에서 서브쿼리를 사용할 때 일반적으로 해당 서브쿼리의 결과 컬럼은 1개가 와야함
SELECT employee_number, name, age
FROM employee
WHERE department_code = (
    SELECT department_code 
...
ON  E.department_code = D.departemnt_code;

-- 서브쿼리를 FROM 절에서 사용할 땐 3개 이상의 테이블을 조인ㄴ해서 결과를 얻고자 할 때 아주 유용하게 사용됨